class BallClock {
  ...
  mapTransformation (original, map) {
    let array = []
    map.forEach((ballNum) => { array.push(original[ballNum-1]) })
    // console.log('the map said take the ', ballNum, ' number ball at index ', ballNum-1, ' and at that index in the original is ball ', originalMain[ballNum-1])
    return array
  }
  
  powerTransform (originals, maps, numTimes, arrayName=null) {
    let combinedMap = maps.main
    let combinedState = originals.main
    let mainLength = maps.main.length
    if (arrayName) {
      let arrayOriginal = originals[arrayName]
      let arrayMap = maps[arrayName]
      combinedMap = combinedMap.concat(arrayMap)
      combinedState = combinedState.concat(originals)
    }
    for (let i = 0; i < numTimes; i++) {
      combinedState = this.mapTransformation(combinedState, combinedMap)
      console.log(i+1, 'th iteration on ', arrayName, ': ', combinedState)
    }
    let main = combinedState
    let array = null
    if (arrayName) {
      main = combinedState.slice(0, mainLength)
      array = combinedState.slice(mainLength, combinedState.length)
    }
    return { main, array }
  }
  ...
  transformToMin (numBalls, numMinutes) {
    ...
    if (mults.five) {
      console.log('maps.five ', maps.five)
  ...
  countCycles (numBalls) {
    ...
    while (!this.arraysEqual(main, this.original)) {
      main = this.mapTransformation(main, cycleMap)
      cycles++
    }
    return cycles*12*60
  }
  ...
  modeOneComplex (numBalls, numMinutes) {
    /*
    // DOESN'T RUN -- USE NAIVE ALGORITHM
    ...
    this.printTime(() => {
      let result = this.transformToMin(numBalls, numMinutes)
      console.log(result)
...
// ballClock.runTests()
ballClock.run(127)class BallClock {
